...
| HARD    | 77        | ארכיטקטורה, integration, agent, PRD |
...
**כלל חוסם:** כשמוצגת הצעת שדרוג/הורדה — **עצירה מוחלטת**. אסור להמשיך בשום עבודה. לחכות לקלט מהמשתמש. חל ללא קשר להרשאות, bypass, או רמת אוטומציה. ההצעה = חלון אישור חוסם.
...
## שקיפות שינוי רמה בסוכנים (חובה)

כאשר **כל סוכן או סאב-אייג'נט** (Explore, Plan, general-purpose, QA, security, או כל אחר) משנה רמת MOS — גם בקפיצה של רמה אחת בלבד — **חובה** להודיע למשתמש:

```
🔄 שינוי רמת MOS: [רמה_קודמת] → [רמה_חדשה] (סיבה: [סיבה קצרה])
```

כללים:
- חל על **כל** שינוי רמה — כולל ±1 רמות.
- סוכנים **רשאים** לשנות רמה ביוזמתם כשמורכבות המשימה דורשת — זה מותר וצפוי.
- אבל **שינוי שקט אסור**. כל שינוי חייב הודעה לפני המשך עבודה.
- ההודעה **אינפורמטיבית, לא חוסמת** — אין צורך לחכות לאישור. להודיע ולהמשיך.
- אם סוכן הופעל ברמה מסוימת אך המשימה מתגלה כפשוטה/מורכבת יותר — חובה להודיע בתגובה הראשונה לאחר זיהוי אי-ההתאמה.

דוגמה:
```
🔄 שינוי רמת MOS: SIMPLE → MEDIUM (סיבה: משימה כוללת רפקטור רב-קבצי עם תלויות צולבות)
...
3. **שקיפות שינוי רמה** — כל שינוי רמה, כל סוכן, גם ±1 רמות.
4. **SessionStart hook** מדפיס `MOS: active` כאישור.
5. אם בלוק MOS דולג בתחילת session, הפרומפט הראשון מפעיל אותו לפני מענה.
